...
<div align="center">
  <img src="public/logo.png" alt="Zhiying Cinema Logo" width="120">
</div>

> 🎬 **Zhiying Cinema** 是一个开箱即用的、跨平台的影视聚合播放器。它基于 **Next.js 14** + **Tailwind&nbsp;CSS** + **TypeScript** 构建，支持多资源搜索、在线播放、收藏同步、播放记录、本地/云端存储，让你可以随时随地畅享海量免费影视内容。
...
docker run -d --name tingbao-cinema -p 3000:3000 ghcr.io/senshinya/moontv:latest
```
...
services:
  tingbao-cinema-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://tingbao-cinema-redis:6379
      - NEXT_PUBLIC_ENABLE_REGISTER=true
    networks:
      - tingbao-cinema-network
    depends_on:
      - tingbao-cinema-redis
    # 如需自定义配置，可挂载文件
    # volumes:
    #   - ./config.json:/app/config.json:ro
  tingbao-cinema-redis:
    image: redis
    container_name: tingbao-cinema-redis
    restart: unless-stopped
    networks:
      - tingbao-cinema-network
    # 如需持久化
...
| SITE_NAME                   | 站点名称                                                    | 任意字符串                       | Zhiying Cinema                                                                                                             |
...
Zhiying Cinema 支持标准的苹果 CMS V10 API 格式。
...
[MIT](LICENSE) © 2025 Zhiying Cinema & Contributors
